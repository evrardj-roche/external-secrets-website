...
{{< readfile file=../../snippets/generator-mfa.yaml code="true" lang="yaml" >}}
...
> [!WARNING] Usage of the token might fail on first try if it JUST expiredIt is possible that from requesting the token to actually using it, the token might be already out of date if timeLeft was
