...
![PushSecretBackup](/img/diagrams-pushsecret-backup.png)
...
{{< readfile file=/snippets/full-pushsecret-no-key-no-property.yaml code="true" lang="yaml" >}}
...
{{< readfile file=/snippets/full-pushsecret-no-key-with-property.yaml code="true" lang="yaml" >}}
...
You can also set `data[*].conversionStrategy: ReverseUnicode` to reverse the invalid character replaced by the `conversionStrategy: Unicode` configuration in the `ExternalSecret` object as [documented here](getallsecrets.md#avoiding-name-conflicts).
...
{{< readfile file=/snippets/pushsecret-generator-rotation-example.yaml code="true" lang="yaml" >}}
