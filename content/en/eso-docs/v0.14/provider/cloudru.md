...
linkTitle = "Cloud.ru Secret Manager"
weight = 730
+++
...
* Create a [SecretStore](../api/core-resources/secretstore.md) pointing to `csm-secret` k8s Secret:
    ```yaml
...
#### Create an [ExternalSecret](../api/core-resources/externalsecret.md) pointing to SecretStore.
  * Classic, non-json:
    ```yaml
    apiVersion: external-secrets.io/v1
    kind: ExternalSecret
