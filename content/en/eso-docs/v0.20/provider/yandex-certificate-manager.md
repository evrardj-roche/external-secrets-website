...
linkTitle = "Yandex Certificate Manager"
weight = 810
+++
...
```
* Create a [SecretStore](../api/core-resources/secretstore.md) pointing to `yc-auth` k8s secret:
```yaml
...
```
* Create an [ExternalSecret](../api/core-resources/externalsecret.md) pointing to `secret-store` and the certificate in Certificate Manager:
```yaml
apiVersion: external-secrets.io/v1
kind: ExternalSecret
metadata:
