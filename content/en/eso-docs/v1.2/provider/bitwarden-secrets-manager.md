...
linkTitle = "Bitwarden Secrets Manager"
weight = 110
+++
...
{{< readfile file=../snippets/bitwarden-secrets-manager-secret-store.yaml code="true" lang="yaml" >}}

The api url and identity url are optional. The secret should contain the token for the Machine account for bitwarden.

> [!NOTE]Make sure that the machine account has Read-Write access to the Project that the secrets are in.

> [!NOTE]A secret store is organization/project dependent. Meaning a 1 store == 1 organization/project. This is so that we ensure
